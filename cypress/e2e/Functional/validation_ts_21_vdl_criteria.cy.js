...
TC_49: Verify on clicking vdl criteria link the scroll to title works as expected
*/
...
describe("Verification of Validation Page TS 21 VDL Block Title Links", () => {
  viewports.forEach(({ device, viewport }) => {
    it(`Verify on clicking vdl criteria link the scroll to title works as expected on ${device}`, () => {
      // Set the viewport for each test case
      cy.viewport(viewport);
      validationPage.verifyVDLBlockTitle(
        device,
        validationPage.getVDLCriteriaLink,
        validationPage.getVDLCriteria,
        "#block-vdl-vdlcriteria"
      );
    });
  });
});
