...
TC_57: Verify on clicking terms of use link the scroll to title works as expected
*/
...
describe("Verification of Terms and Conditions Page TS 26 Terms Title Links", () => {
  viewports.forEach(({ device, viewport }) => {
    it(`Verify on clicking terms of use link the scroll to title works as expected on ${device}`, () => {
      // Set the viewport for each test case
      cy.viewport(viewport);
      termsConditionPage.verifyTermsBlockTitle(
        termsConditionPage.getTermsUseLink,
        termsConditionPage.getTermsUse,
        "#block-vdl-termsofuse"
      );
    });
  });
});
